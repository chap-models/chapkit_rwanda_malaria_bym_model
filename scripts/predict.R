...
#
# Column mapping (was MLproject adapters; applied in-script for chapkit):
#   Cases    = disease_cases
#   E        = population
#   month    = month
#   ID_year  = year
...
predict_chap <- function(model_fn, hist_fn, future_fn, preds_fn, config_fn, polygons_fn) {
  ...
  # chap-core canonical -> Kigali internal R names. Equivalent to the original
  # MLproject adapter map; chapkit does not apply adapters, so we rename here.
  # Tolerant of missing source columns (chapkit test's synth generator emits
  # only time_period and not year/month) - mirrors chapkit_ewars_model's
  # apply_adapters pattern.
  rename_map <- c(
    "disease_cases" = "Cases",
    "population"    = "E",
    "year"          = "ID_year",
  ...
  # Derive ID_year / month from time_period if not already present.
  # chap-core's canonical CSV ships year + month columns, but chapkit test's
  # synth generator only emits time_period (YYYY-MM).
  if (!("ID_year" %in% names(df))) {
